...
int buffer_encrypt(const char *password, const char *inbuf, int inlen, char *outbuf, int *poutlen)
{
    return buffer_do_crypt(Encryption, (char*)password, (char*)inbuf, inlen, outbuf, poutlen);
}

int buffer_decrypt(const char *password, const char *inbuf, int inlen, char *outbuf, int *poutlen)
{
    return buffer_do_crypt(Decryption, (char*)password, (char*)inbuf, inlen, outbuf, poutlen);
}
...
int buffer_do_crypt(OperationType operation_type, char *password, char *inbuf, int inlen, char *outbuf, int *poutlen)
{
    ...
    EVP_CipherInit_ex(&ctx, NULL, NULL, (unsigned char *)password, NULL, operation_type);
    ...
    if (!EVP_CipherFinal_ex(&ctx, (unsigned char *)outbuf + outlen, poutlen)) {
        /* Error */
        EVP_CIPHER_CTX_cleanup(&ctx);
        return -1;
    }
